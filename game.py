...
def showLeaderboard():
    ...
    y = 100
    for rank, (place, score) in enumerate(leaderboard[:5], start=1):  # Display top 5 entries
        text = font.render(f"{rank} ) : {score}", True, white)
        display.blit(text, (50, y))
        y += 30

# Function to update leaderboard
def updateLeaderboard(place, player_score):
    leaderboard.append((place, player_score))
...
class Stack:
    ...
    def pushToStack(self):
        global brickW, score, level, stack_count

        b = self.stack[self.initSize - 2]
        b2 = self.stack[self.initSize - 1]
        if b2.x <= b.x and not (b2.x + b2.w < b.x):
            self.stack[self.initSize - 1].w = self.stack[self.initSize - 1].x + self.stack[
                self.initSize - 1].w - b.x
...
def gameOver():
    ...
    while loop:
        ...
        showScore()
        showLevel()
        showLeaderboard()  # Display leaderboard on game over
...
# Displaying the Level on Screen
def showLevel():
    font = pygame.font.SysFont("Agency FB", 30)
    text = font.render("Level: " + str(level), True, white)
    display.blit(text, (width - 110, 10))
...
# The Main Game Loop
def gameLoop():
    global brickW, brickH, score, colorIndex, speed, level, stack_count
    loop = True
